...
def make_collector(
    ...
    env_loop, episode_ids, dead = (None,) * 3
    num_steps, num_episodes, buffer, to_log, pbar = (None,) * 5
    ...
    while True:
        with torch.no_grad():
            all_obs, act, rew, end, trunc = env_loop.send(1)    # XXX: only once
        ...
        for i in range(num_envs):
            ...
            if add_to_dataset:
                ep = Episode(*(torch.cat(x, dim=0) for x in zip(*buffer[i]))).to("cpu")
